...
const TestsList = () => {
  ...
  const [answeredTestOffice, setAnsweredTestOffice] = useState(undefined);
  ...
  const getAnsweredTest = async () => {
    try {
      ...
      data.map((test) => {
        ...
        } else if (test.test === "ofimatica") {
          return setAnsweredTestOffice(test);
        }
        return null;
      });
    } catch (error) {
